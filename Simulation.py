...
class Simulation():
    ...
    def safetyFilter(self, X, Udis):
        ...
        prob = cp.Problem(cp.Minimize((1/2)*cp.quad_form(usafe, P) - q.T@usafe), [G @ usafe <= h])
        prob.solve()
        ...
        return torch.tensor(usafe.value, dtype=float)
...
def testSafetyFilter():
    ...
    X = torch.tensor([1.2, 1.5]).reshape(1,sim.sys.D) #sim.gen.x_min.reshape(1,sim.gen.D)
    U = torch.tensor([2]).reshape(1,sim.sys.M) #sim.gen.uMap(torch.tensor(sim.gen.u_min).reshape(1,sim.gen.M))
