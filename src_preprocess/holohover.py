...
from src_preprocess import Data
    ...
    def cropData(self, plot=False):
        ...
        for exp in self.data.exps:
            ...
            if plot:
                ...
                plt.vlines(tx[exp][lower_x_idx], ymin=-3.3, ymax=3.3, colors="r")
                plt.vlines(tx[exp][upper_x_idx], ymin=-3.3, ymax=3.3, colors="r")
            ...
            datas = [x[exp][lower_x_idx:upper_x_idx+1,:], tx[exp][lower_x_idx:upper_x_idx+1], 
                     u[exp][lower_u_idx:upper_u_idx+1,:], tu[exp][lower_u_idx:upper_u_idx+1]]
            self.data.set(names=["x", "tx", "u", "tu"], datas=datas)
